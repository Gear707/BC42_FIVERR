...
const schema = yup.object({
    ...
    maCongViec: yup.string()
        .required("Job index is required"),
...
function HiredJobs({ userInfo }) {
    ...
    const getItemsForCurrentPage = () => {
        ...
        return hiredJobs.slice(firstIndex, lastIndex);
    };
            ...
            {hiredJobs?.length >= 1 ?
                (
                    <>
                        ...
                        <Pagination defaultCurrent={currentPage} total={hiredJobs.length}
                            defaultPageSize={jobsPerPage} showTotal={(total) => `Total ${total} items`}
